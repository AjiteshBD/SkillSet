...
export default function UserCollections() {
  ...
  async function loadNFTS(){
    ...
    const signer = provider.getSigner()
      ...
      const team = items.filter(async(i)=>{await getFlow(i.seller).deposit>0})
      setTeams(team)
      setNFts(items)
          ...
          <div key={i} className='border shadow rounded-x1 overflow-hidden'>
            ...
            <div className='p-4'>
              ...
              <div style={{height:'40px'}}>
                  <p className='text-gray-400 text-white'>{nft.skills}</p>

                  <p className='text-gray-400 text-white'>{nft.skillscore}</p>
              </div>
